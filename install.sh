...
PKG="bundle_loader"
GIT_URL="https://github.com/madcato/bundle_loader.git"
...
install_from_git(){
    ...
    if [ -e "${INSTALL_DIR}" ]; then
        ...
        chmod 755 ${INSTALL_DIR}/bender 2>/dev/null
        sudo ln -sf ${INSTALL_DIR}/bender /usr/local/bin/bender
    fi
}
...
if [ -f "${INSTALL_DIR}/bundle_loader" ]; then
    echo ""
    echo "Done!"
else
    echo >&2 ""
    echo >&2 "Something went wrong. ${INSTALL_DIR}/bender not found"
    echo >&2 ""
    exit 1
fi
